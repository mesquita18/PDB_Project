...
class Aluno(models.Model):
    permission_classes = [IsAuthenticated]
    user = models.OneToOneField(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='aluno_profile')
...
class Turma(models.Model):
    ...
    professor = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='turmas_professor')
...
class Nota(models.Model):
    ...
    def media_final(self):
            # Se não há média, não é possível calcular
            if self.media is None:
                return None
            # Se há final, calcula a média final
            if self.final is not None:
                try:
                    return round((float(self.media) + float(self.final)) / 2, 2)
                except Exception:
                    return None
            # Se não há final, mas a média já garante aprovação, retorne a média
            if self.media >= 7.0:
                return self.media
            # Sem final e média insuficiente para aprovação, não há média final definida
            return None
